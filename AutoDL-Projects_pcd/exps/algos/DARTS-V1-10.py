...
def main(xargs):
  ...
  decay_list=[1]+[1/(math.log(softmax_temp+softmax_decay*(i//decay_interval))/0.000399) for i in range(1,cycle_interval//2+1)]
  decay_list=decay_list+decay_list[::-1][1:-1]
  final_decay_list=[1/(math.log(softmax_temp+softmax_decay*(i//decay_interval))/0.000399) for i in range(1,cycle_interval+1)]
  entropy_n=0
  warmup=5
