...
def search_func(xloader, network, criterion, scheduler, w_optimizer, a_optimizer, epoch_str, print_freq, logger, gradient_clip,warmup,epoch,softmax_temp):
  ...
  for step, (base_inputs, base_targets, arch_inputs, arch_targets) in enumerate(xloader):
    ...
    arch_loss.backward()
    #if epoch>=warmup:
    if not math.isclose(softmax_temp,1.0):
...
def main(xargs):
  ...
  cycle_interval=10
  decay_target=0.001
  softmax_decay=(softmax_temp-decay_target)/(cycle_interval//2)
  #decay_list=[1]+[1/(math.log(softmax_temp+softmax_decay*(i//decay_interval))/0.000395) for i in range(1,cycle_interval)]
  decay_list=[1]+[1-i*softmax_decay for i in range(1,cycle_interval//2+1)]
  decay_list=decay_list+decay_list[::-1][1:-1]
  softmax_decay=(softmax_temp-decay_target/2)/(cycle_interval)
  final_decay_list=[1-i*softmax_decay for i in range(1,cycle_interval+1)]
  entropy_n=0
  warmup=10

  for epoch in range(start_epoch, total_epoch):
    #if epoch>0 and epoch%decay_interval==0: new_temp=1/(math.log(softmax_temp+softmax_decay*(epoch//decay_interval))/0.000395)
    #new_temp=decay_list[epoch%cycle_interval]
    if epoch>=warmup:
    ...
    logger.log('\n[Search the {:}-th epoch] {:}, LR={:}, temperature={:}'.format(epoch_str, need_time, min(w_scheduler.get_lr()),new_temp))
