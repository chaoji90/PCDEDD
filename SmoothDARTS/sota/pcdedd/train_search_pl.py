...
from sota.cnnspp.model_search_cyc import Network
from optimizers.darts.architect_cyc import Architect
from sota.cnnspp.spaces import spaces_dict
...
parser.add_argument('--decay_strength',type=float,default=0.12,help='start value of C')
    ...
    for epoch in range(args.epochs):
        if epoch==warmup or (epoch>warmup and epoch%decay_interval==0):
          with torch.no_grad():
              ...
              entropy_n=entropy(softmax_out,axis=1).sum()/softmax_out.shape[0]
              if epoch<=39:
                  #0.02-0.04/0.12-0.24
                  softmax_decay=decay_strength*entropy_n*decay_rate+(1-decay_rate)*softmax_decay
              else:
                  decay_interval=1
                  softmax_decay=2*decay_strength*entropy_n*decay_rate+(1-decay_rate)*softmax_decay
        ...
        # training
        train_acc, train_obj = train(train_queue, valid_queue, model, architect, criterion, optimizer, lr, perturb_alpha, epsilon_alpha,new_temp)
        ...
        with torch.no_grad():
            ...
            logging.info('softmax_temp=%f',new_temp)

        utils.save(model, os.path.join(args.save, 'weights.pt'))
    writer.close()


def train(train_queue, valid_queue, model, architect, criterion, optimizer, lr, perturb_alpha, epsilon_alpha,temp):
    ...
    for step, (input, target) in enumerate(train_queue):
        ...
        architect.step(input, target, input_search, target_search, lr, optimizer, args.unrolled,temp)
        optimizer.zero_grad()
        ...
        #logits = model(input, temp,updateType='weight')
        logits,_ = model(input, temp)
