...
def search_func(xloader, network, criterion, scheduler, w_optimizer, a_optimizer, epoch_str, print_freq, logger, gradient_clip,warmup,epoch,softmax_temp):
  ...
  for step, (base_inputs, base_targets, arch_inputs, arch_targets) in enumerate(xloader):
    ...
    _, logits, _= network(base_inputs,softmax_temp)
    ...
    #if epoch<30: _, logits, alphas = network(arch_inputs)
    #elif epoch<40: _, logits, alphas = network(arch_inputs,0.1)
    #else: _, logits, alphas = network(arch_inputs,0.01)
    #_, logits = network(arch_inputs,1-0.9*epoch/49)

    alphas.register_hook(softmaxOutGrad)
    arch_loss = criterion(logits, arch_targets)
    arch_loss.backward()
    if warmup<epoch:
        #alphas_smooth=torch.nn.functional.softmax(network.module.arch_parameters,dim=-1)
        #alphas_smooth=torch.nn.functional.softmax(network.module.arch_parameters/(softmax_temp*100),dim=-1)
...
def valid_func(xloader, network, criterion,new_temp):
  ...
  with torch.no_grad():
    for step, (arch_inputs, arch_targets) in enumerate(xloader):
      ...
      data_time.update(time.time() - end)
      # prediction
      _, logits, _= network(arch_inputs,new_temp)
      arch_loss = criterion(logits, arch_targets)
      # record
      arch_prec1, arch_prec5 = obtain_accuracy(logits.data, arch_targets.data, topk=(1, 5))
      arch_losses.update(arch_loss.item(),  arch_inputs.size(0))
...
def main(xargs):
  ...
  decay_rate=0.5
  warmup=10
  #decay_list=[1]+[1/(math.log(softmax_temp+softmax_decay*(i//decay_interval))/0.000395) for i in range(1,cycle_interval)]
  #decay_list=[1]+[1/(math.log(softmax_temp+softmax_decay*(i//decay_interval))/0.000395) for i in range(1,cycle_interval//2+1)]
  #decay_list=decay_list+decay_list[::-1][1:-1]
  #final_decay_list=[1/(math.log(softmax_temp+softmax_decay*(i//decay_interval))/0.000395) for i in range(1,cycle_interval+1)]

  for epoch in range(start_epoch, total_epoch):
    if epoch==warmup or (epoch>warmup and epoch%decay_interval==0): 
        with torch.no_grad():
            ...
            if epoch<=39:
                softmax_decay=0.04*entropy_n*decay_rate+(1-decay_rate)*softmax_decay
            else: 
                decay_interval=1
                softmax_decay=0.08*entropy_n*decay_rate+(1-decay_rate)*softmax_decay
            new_temp=0.000399/math.log(softmax_temp+softmax_decay*decay_count)
            decay_count+=1
    w_scheduler.update(epoch, 0.0)
    need_time = 'Time Left: {:}'.format( convert_secs2time(epoch_time.val * (total_epoch-epoch), True) )
    epoch_str = '{:03d}-{:03d}'.format(epoch, total_epoch)
    logger.log('\n[Search the {:}-th epoch] {:}, LR={:}, temperature={:}, entropy_n={:}'.format(epoch_str, need_time, min(w_scheduler.get_lr()),new_temp,entropy_n))

    search_w_loss, search_w_top1, search_w_top5 = search_func(search_loader, network, criterion, w_scheduler, w_optimizer, a_optimizer, epoch_str, xargs.print_freq, logger, xargs.gradient_clip,warmup,epoch,new_temp)
    ...
    with torch.no_grad():
        softmax_out=torch.nn.functional.softmax(search_model.arch_parameters/new_temp, dim=-1).cpu().data.numpy()
        entropy_p=entropy(softmax_out,axis=1).sum()
    logger.log('entropy={:}'.format(entropy_p))
